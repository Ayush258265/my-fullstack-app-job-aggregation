...
const JobDetails = ({ job, loading }) => {
  const { user } = useAuth();
  const [showApplyPopup, setShowApplyPopup] = useState(false);
  const hasClickedApply = useRef(false); // ✅ NEW

  // ✅ NEW: watch for user coming back to this tab after clicking Apply
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible' && hasClickedApply.current) {
        setShowApplyPopup(true);
  ...
  // ✅ CHANGED: no longer opens popup immediately
  const handleApply = () => {
    if (applyUrl) {
      window.open(applyUrl, '_blank');
      hasClickedApply.current = true; // ✅ NEW
    } else {
      setShowApplyPopup(true); // ✅ NEW: fallback if no applyUrl
    }
  };
